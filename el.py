...
word2vec = Word2Vec.load('../../../data/kg/word2vec_baike/word2vec_baike')
...
id2kb = {}
with open('../ccks2019_el/kb_data', encoding='utf-8') as (f):
    for l in tqdm(f):
        _ = json.loads(l)
        subject_id = _['subject_id']
        subject_alias = set([_['subject']] + _.get('alias', []))
        ...
        _['data'].append({
            'predicate': '名称',
            'object': '、'.join(subject_alias)
        })
        subject_desc = '\n'.join(
            '%s：%s' % (i['predicate'], i['object']) for i in _['data']
        )
...
with open('../ccks2019_el/train.json', encoding='utf-8') as (f):
    for l in tqdm(f):
        _ = json.loads(l)
        train_data.append({
            'text': _['text'],
...
words = set(list(words_to_pred.keys()) + list(words_to_remove.keys()))
words = {
...
if not os.path.exists('../random_order_train.json'):
    ...
    json.dump(random_order, open('../random_order_train.json', 'w', encoding='utf-8'), indent=4)
else:
    random_order = json.load(open('../random_order_train.json', encoding='utf-8'))
...
for k, v in subjects.items():
    v = {i: j for i, j in v.items() if j >= 2}
    if v:
...
with open('../ccks2019_el/develop.json', encoding='utf-8') as f:
    for l in tqdm(f):
        _ = json.loads(l)
        test_data.append(_)
    ...
    def __iter__(self):
        while True:
            ...
            for i in idxs:
                ...
                x1 = [char2id.get(c, 1) for c in text]  # query.char_ids
                s1, s2 = np.zeros(len(text)), np.zeros(len(text))  # 记录在 kg 中出现过的 entity 的 start_id ,end_id 的指针表示
                mds = {}
                for md in d['mention_data']:  # 构建 start_id ,end_id 的指针表示
                ...
                if mds:
                    ...
                    x2 = ''.join(x2_words)
                    y2 = isin_feature(text, x2)  # text 中的token是否在 x2 中出现过 ,char_level
                    y3 = isin_feature(text_words, x2_words)  # text_words 中的 token 是否在 x2_words 中出现过，word_level
                    y4 = is_match_objects(text, object_regex)  # 标记 text 中的能够被 object_regex 匹配的部分
                    y = np.vstack([y1, y2, y3, y4]).T
                    x2 = [char2id.get(c, 1) for c in x2]  # document.char_ids
                    pre_subjects = search_subjects(d['text'])  # 在过滤的到的实体库里面进行匹配，能匹配上的位置都用1标识，start==>pres1 end==>pres2
                    ...
                    X1V.append(text_words)  # query.words
                    X2V.append(x2_words)  # document.words
                    S1.append(s1)  # 记录在 kg 中出所 mention  entitys 的 start_ids
                    S2.append(s2)  # 记录在 kg 中出所 mention entitys 的 ends_ids
                    PRES1.append(pres1)  # 在过滤的到的实体库里面进行匹配，能匹配上的位置都用1标识，start==>pres1
                    PRES2.append(pres2)  # 在过滤的到的实体库里面进行匹配，能匹配上的位置都用1标识，end==>pres2
                    Y.append(y)
                    T.append(t)
...
pres1 = Lambda(lambda x: K.expand_dims(x, 2), name='pres1')(pres1)  # [B,CT] = > [B,CT,1]
pres2 = Lambda(lambda x: K.expand_dims(x, 2), name='pres2')(pres2)
x1 = Concatenate()([x1, pres1, pres2])  # ([B,CT,CH],[B,CT,1][B,CT,1])=> [B,CT,CH+2]
x1 = Lambda(lambda x: x[0] * x[1], name='x1')([x1, x1_mask])  # 其实 lambda 这里的主要作用就是实现一些 batch_wise 的 操作<==这个理解是错误的【X】
# https://kexue.fm/archives/6810#%E7%BB%93%E5%90%88%EF%BC%9A%E5%8F%8C%E5%90%91RNN
...
ys = Lambda(lambda x: K.sum(x[0] * x[1][..., :1], 1) / K.sum(x[1][..., :1], 1), name='ys')([x1, y])
...
# 这里可以去看一下层中层与 mask 那篇文章 https://kexue.fm/archives/6810
...
def extract_items(text_in):
    ...
    if _subjects:
        ...
        if _X2:
            ...
            for k, v in groupby(list(zip(_S, scores)), key=lambda s: s[0]):
                ks = _IDXS[k]
                vs = [j[1] for j in v]
                if np.max(vs) < 0.1:
                    continue
